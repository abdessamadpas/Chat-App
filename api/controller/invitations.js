...
const setInvitations  = async (senderId, receiverId) => {
    ...
    const checkInvitation = await User.find({_id: receiver._id, invitations: senderId._id});
    ...
    if (checkInvitation.length > 0){
        return res.status(400).json({message: 'u already send the invitation  to this user😒'});
    }
