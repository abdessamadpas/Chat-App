...
import { MdNotificationsNone , MdOutlineExpandMore, MdMoreVert} from "react-icons/md";
const attachments = [
  {
...
const members = [
  ...
  },
  {
    ...
    date : 'last seen recently',
    leader : false,
    image : "https://www.creative-tim.com/learning-lab/tailwind-starter-kit/img/team-2-800x800.jpg"
      ...
      <div className='mt-10'>
        ...
        <div className='flex flex-col'>
          {attachments.map((element, index)=>(
            <div key={index} className='flex  justify-start items-center mt-5 gap-3'>
              <div className='  bg-white w-10 h-10 flex justify-center items-center rounded-full'>
                <p className=' text-xs  text-text-main-color  font-medium'>{element.type}</p>
              </div>
              <div className=''>
                <p className=' font-bold text-sm text-gray-800'>{element.name}</p>
                <p className='font-light  text-xs text-gray-400 tracking-wide'>{element.size} mb • {element.date}</p>
              </div>
            </div>
          ))}
          ...
          {members.map((element, index)=>(
            <div key={index} className=' flex justify-between  gap-9  mt-5  '>
              <div className='flex gap-2'>
                ...
                <div >
                  <p className=' font-bold text-sm text-gray-800'>{element.name}</p>
                  <p className='font-light  text-xs text-gray-400 tracking-wide'>{element.date}</p>
                </div>
              </div>
              {element.leader ? 
